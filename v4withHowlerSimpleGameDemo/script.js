...
// Try declaring sounds here for howler.js
let correctSound, bounceSound
  ...
  if (numRemaining <= 0) {
    ...
    for (i = 0; i < ballGroup.length; i++) {
      ...
      if (thisBall.collides(bars) || thisBall.collides(striker)) {
        ...
        // bounceSound.setVolume(min(thisVel / 10, 0.5)); // p5 sound
        bounceSound.volume(min(thisVel / 10, 0.5)); // Howler sound
        bounceSound.play();
      }
    }
  }
...
// This next function just preloads thse sounds, so that they play quicker
function preload() {
  // p5js sound commands
  // soundFormats('mp3');
...
function mousePressed() {
  ...
  if (mouseHasBeenPressed == 0) {
    // bounceSound.setVolume(0.01); // p5 sound
    bounceSound.volume(0.01); // Howler sound
    bounceSound.play();
  }
